...
public class SOGunControlsYt : MonoBehaviour
{
    ...
    //public AudioSource gunOutOfAmmo;// SOUND OF THE WEAPON WHEN OUT OF AMMO;
    //public AudioSource Reloading;// SOUND OF THE WEAPON WHEN RELOADING;
    ...
    public void UpdateFunctions()
    {
        if (Equipped == true)
        {
            //recoilControls.sOGunControls = sOGunControlsThis;
            RecoilLogic();
            AmmunitionCheck();
            ReloadWeapon();
    ...
    {
        if (AbleToFire == true && selectFireAutomatic == true && Equipped == true)
        {
            
            InvokeRepeating("Shooting", gunVariables.DelayBeforeFirstShot, gunVariables.fireRate);

        }
        else if (AbleToFire == true && selectFireAutomatic == false && Equipped == true)
        {
    ...
    public void AimDownSight()
    {
        if(Input.GetKeyDown(KeyCode.Mouse1) && CurrentlyADS == false)
        {
          ...
          //  GunRb.transform.localPosition = itemPosition.ADSPosition;
          //  GunRb.transform.localEulerAngles = itemPosition.ADSRotation;
           // GunRb.transform.localScale = Vector3.one;
        }
        else if(Input.GetKeyDown(KeyCode.Mouse1) && CurrentlyADS == true)
        {
           ...
           // GunRb.transform.localPosition = itemPosition.equippedPosition;
           // GunRb.transform.localEulerAngles = itemPosition.equippedRotation;
           // GunRb.transform.localScale = Vector3.one;
        }
